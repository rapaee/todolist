                        ...
                        <ul class="list-group mb-4" id="todo-list">
                            ...
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <input type="text" class="form-control edit-input" style="display: none;"
                                    value="{{ $item->task }}">
                                <div class="btn-group">
                                    <form action="{{ route('todolist.delete',['id'=>$item->id]) }}" method="POST" onsubmit="return confirm('Yakin untuk menghapus data ini?')">
                                        @csrf
                                        @method('delete')
                                ...
                                <form action="{{ route('todolist.update',['id'=>$item->id])}}" method="POST">
                                    ...
                                    <div class="d-flex">
                                        <div class="radio px-2">
                                            <label>
                                                <input type="radio" value="1" name="id_done" {{ $item->id_done == '1'?'checked':'' }}> Selesai
                                            </label>
                                        </div>
                                        <div class="radio">
                                            <label>
                                                <input type="radio" value="0" name="id_done" {{ $item->id_done == '0'?'checked':'' }}> Belum
                                            </label>
                                        </div>
                                    </div>
                                </form>
